...
const SubscriptionTable = ({
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      ...
      >
        <Icon name={config.icon} size={14} className="mr-1 text-white" />
  ...
  const getPlanTypeBadge = (planType) => {
    const typeConfig = {
      starter: { bgColor: "bg-blue-50", textColor: "text-white" },
      professional: { bgColor: "bg-purple-50", textColor: "text-white" },
      business: { bgColor: "bg-green-50", textColor: "text-white" },
      enterprise: { bgColor: "bg-orange-50", textColor: "text-white" },
    };
    ...
    return (
      <span
        className={`inline-flex px-2 py-1 rounded text-xs font-medium ${config.bgColor} ${config.textColor}`}
      >
    ...
    return [...subscriptions].sort((a, b) => {
      ...
      switch (sortConfig.key) {
        ...
        case "mrr":
          ...
          break;
        case "nextBilling":
          aValue = a.nextBillingDate
            ? new Date(a.nextBillingDate)
            : new Date(0);
          bValue = b.nextBillingDate
  ...
  const searchFilteredSubscriptions = sortedSubscriptions.filter(
    (subscription) => {
      ...
      <div className="bg-surface rounded-lg border border-border-light shadow-card overflow-hidden">
        <div className="p-8 flex flex-col items-center justify-center">
          ...
          <p className="text-text-secondary text-center max-w-md mb-6">
            ...
            search criteria or create a new subscription.
          </p>
                  ...
                  <div className="flex items-center justify-end space-x-2">
                    ...
                    {subscription.status === "active" && (
                      <>
                        <button
                          onClick={() =>
                            onSubscriptionAction("upgrade", subscription.id)
                          }
                        ...
                        <button
                          onClick={() =>
                            onSubscriptionAction("pause", subscription.id)
                          }
                    ...
                    {subscription.status === "paused" && (
                      <button
                        onClick={() =>
                          onSubscriptionAction("resume", subscription.id)
                        }
                        className="p-1.5 text-secondary-600 hover:text-success hover:bg-success-50 rounded-lg transition-colors duration-200"
                    ...
                    <div className="relative group">
                      ...
                      <div className="absolute right-0 mt-2 w-48 bg-surface rounded-lg shadow-modal border border-border-light z-10 hidden group-hover:block">
                        <div className="py-1">
                          <button
                            onClick={() =>
                              onSubscriptionAction("downgrade", subscription.id)
                            }
                          ...
                          <button
                            onClick={() =>
                              onSubscriptionAction("cancel", subscription.id)
                            }
      ...
      {subscriptions.length > 0 && searchFilteredSubscriptions.length === 0 && (
        <div className="p-8 text-center">
          <Icon
            name="Search"
            size={24}
            className="text-secondary-400 mx-auto mb-3"
          />
      ...
      <div className="bg-secondary-50 px-6 py-4 border-t border-border-light flex items-center justify-between">
        <div className="text-sm text-text-secondary">
          ...
          <span className="font-medium">
            {searchFilteredSubscriptions.length}
          </span>{" "}
