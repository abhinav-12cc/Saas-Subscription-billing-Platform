            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-text-primary">
                    {subscription?.metadata?.salesRep || "N/A"}
                  </p>
                </div>
              </div>
            </div>
                ...
                {subscription?.trial?.isTrialActive && (
                  <div>
                    ...
                    <p className="text-sm text-accent-600 font-medium">
                      {formatDate(subscription?.trial?.trialEndDate)}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-secondary-50 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-medium text-text-primary">
                    {subscription?.usage?.current?.toLocaleString()} /{" "}
                    {subscription?.usage?.limit?.toLocaleString()}
                  </span>
                </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {usageHistory.map((record, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between p-3 bg-surface border border-border-light rounded-lg"
                  >
                    <div>
                      ...
                      <p className="text-xs text-text-tertiary">
                        {Math.round((record.usage / record.limit) * 100)}% of
                        limit used
                      </p>
                    </div>
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    {subscription?.paymentMethod?.type === "credit_card" ? (
                      ...
                      <>
                        <p className="text-sm font-medium text-text-primary">
                          {subscription?.paymentMethod?.bankName ||
                            "Bank Transfer"}
                        </p>
                        <p className="text-xs text-text-secondary">
                          {subscription?.paymentMethod?.accountLast4 &&
                            `•••• ${subscription?.paymentMethod?.accountLast4}`}
                        </p>
                      </>
                    )}
